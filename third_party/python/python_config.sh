...
EXPECTED_PATHS="$script_path/third_party/python/python_include"\
" $script_path/third_party/python/python_lib"
...
# to create junctions on Windows.
  if is_windows; then
    cmd /c "mklink /J third_party\\python\\python_include \"${python_include}\""
    cmd /c "mklink /J third_party\\python\\python_lib \"${python_lib}\""
    #cmd /c "mklink /J third_party\\py\\numpy\\numpy_include \"${numpy_include}\""
  else
    ln -sf "${python_include}" third_party/python/python_include
    ln -sf "${python_lib}" third_party/python/python_lib
